...
public class POO2223 {
	...
	private static void alinea2() {
		...
		String fileName = "digital_objects.txt";
    try (BufferedReader br = new BufferedReader(new FileReader(fileName))) {
        ...
        while ((line = br.readLine()) != null) {
            ...
            if (fields.length == 3) {
                String doi = fields[0].trim();
                String userName = fields[1].trim();
                String sensitivityLevel = fields[2].trim();

                User owner = assetManager.getUser(userName);
                if (owner == null) {
                    owner = assetManager.registerUser(userName);
                }

                SensitivityLevel level = SensitivityLevel.valueOf(sensitivityLevel.toUpperCase());

                assetManager.registerDigitalObject(doi, level, owner);
            }
        }
    } catch (IOException e) {
        e.printStackTrace();
